...
description: |
   基于 ProcessOn 官方 processon-mindmap-generator 1.1.10 的 AI 脑图生成工具，专注于将自然语言、Markdown、长文本、文档、网页、图片文字等内容，一键生成结构清晰、层级分明、可编辑的专业思维导图。
   ...
   本安全版额外要求在 Windows / PowerShell / 中文内容场景优先使用 UTF-8 Markdown 文件和 --markdown-file，避免 stdin 编码导致中文变成 ????。
   注意：本技能主要用于生成思维导图和知识结构类脑图，不适用于流程图、泳道图、时序图、系统架构图、ER 图、Mermaid 图等流程或技术图表生成场景。

author: ProcessOn / 算个文科生吧
version: 1.1.10
---

# LJH ProcessOn Mindmap Safe

本技能同步 `processon-mindmap-generator` 1.1.10 的生成逻辑、结构参数、主题配置和客户端脚本；差异只在执行路径上：Windows 或中文内容默认通过 UTF-8 Markdown 文件提交，降低 PowerShell stdin 乱码风险。
...
2. **Markdown 输出约束 (Markdown Output Constraints)**:
   ...
   - **Windows/环境兼容性与清理**:
     - 在 Windows、PowerShell 或中文内容场景，**必须优先**把 Markdown 正文写入 UTF-8 `.md` 文件，再用 `--markdown-file` 提交，避免管道编码导致中文乱码。
     - **严禁**在项目根目录直接创建临时 `.md` 文件。
     - 临时文件必须放在项目 `.agents/cache/`、`work/` 或系统临时文件夹；一次性文件调用脚本时应追加 `--cleanup-markdown-file`。
     - 非 Windows 且内容不含易受编码影响的中文时，可使用 `stdin` (`--markdown -`) 传递内容。
...
6. **调用脚本同步云端**：在 Windows、PowerShell 或中文内容场景，优先将 Markdown 写入 UTF-8 文件，再使用 `python scripts/processon_mindmap_client.py --markdown-file <file>` 提交到云端；非 Windows 或纯 ASCII 内容可使用 `python3 scripts/processon_mindmap_client.py --markdown -` 从标准输入读取。对于位于项目 `.agents/cache/` 或系统临时文件夹中的临时文件，脚本会自动清理；若需要强制清理任意输入文件，可额外追加 `--cleanup-markdown-file`。
   - **执行示例**：
     ```bash
   ...
   - **PowerShell 中文安全示例**：
     ...
     ## 节点内容
     '@
     python scripts/processon_mindmap_client.py --title "标题" --theme "极简黑白" --structure "mind_free" --markdown-file $path --cleanup-markdown-file
     ```
   - **临时文件兜底示例（跨平台）**：
     ```bash
